...
Trending components for real-time time series data visualization. Provides SVG-based charting with interactive legends, time range controls, cursor inspection, and export functionality.

## Installation

```bash
npm install @talos/trending
```
...
import { TrendChart, TrendLegend, TrendTimeRange, TrendExport } from '@talos/trending';
...
```ts
import { TrendingEngine } from '@talos/trending';
...
A self-contained Web Worker for offloading downsampling and analysis is available at `src/workers/trending.worker.ts`.
